...
def recover_key_bit(s, pos, correct_ctx, key_suffix):
    ptx = b'drzewwaa atakoow'
    for i in range(256):
        key = bytes([0] * pos) + bytes([i]) + key_suffix
...
def recover_key(s):
    ...
    for i in range(BLOCK_SIZE):
        for j in range(i):
            set_key_bit(s, 0, j)
        set_data(s, ptx)
        ctxs.append(read_ctx(s))
    
    key = b''
    for i in range(BLOCK_SIZE - 1, -1, -1):
        key = recover_key_bit(s, i, ctxs[i], key) + key
        print(key)
    return key
